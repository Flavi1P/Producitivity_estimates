"""
Variant of Scripts/gop_vs_ncp_figure.py using a MOVING integration depth that
tracks the mixed-layer depth (MLD), clamped to [50, 200] m, instead of the fixed
0-200 m slab of the parent figure.

  z_bot(pair) = min(200, max(50, max(MLD_i, MLD_j)))

Rationale: the fixed-200 m figure integrates far below the productive layer in
the stratified summer (noisy smoothed line, wide IQR), while 0-40 m under-samples
the deep mixed layer during the spring bloom onset. A moving MLD bottom keeps the
integration matched to the ventilating/productive layer -- deep in winter/spring
(up to the 200 m cap), shallow in summer (down to the 50 m floor). See
Scripts/o2_budget_3902681_movingmld.R for the O2-budget recomputation; everything
else (18-day centered median smoothing, IQR envelope, float 3902681's own
nitrate-drawdown NCP on a twin axis) is unchanged from gop_vs_ncp_figure.py.

Input:  Data/Processed/o2_budget_3902681_movingmld.csv
        Data/Processed/ncp_float_3902681_30d.xlsx
Output: Output/float_3902681_GOP_movingMLD_vs_basin_NCP.png / .pdf
"""
...
O2_CSV         = REPO_ROOT / "Data" / "Processed" / "o2_budget_3902681_movingmld.csv"
FLOAT_NCP_XLSX = REPO_ROOT / "Data" / "Processed" / "ncp_float_3902681_30d.xlsx"
OUT_PNG        = REPO_ROOT / "Output" / "float_3902681_GOP_movingMLD_vs_basin_NCP.png"
OUT_PDF        = REPO_ROOT / "Output" / "float_3902681_GOP_movingMLD_vs_basin_NCP.pdf"
...
def load_gop() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    print(f"[GOP] moving MLD bottom [{MLD_FLOOR},{MLD_CAP}] m, {WINDOW_DAYS}-day "
...
def main() -> int:
    ...
    ax1.plot(gop["date"], gop["gop_mean"], lw=2.0, color=GOP_COLOR, zorder=5,
              label="GOP (flux-corrected O2 budget, moving MLD 50-200 m)")
    ...
    # ---- integration-depth strip (per-pair moving z_bot) ----
    axd.plot(zbot["mtime"], zbot["z_bot_m"], lw=0.8, color=MLD_COLOR, alpha=0.9)
    axd.axhline(MLD_FLOOR, color=ZERO_LINE, lw=0.6, ls="--")
    axd.axhline(MLD_CAP,   color=ZERO_LINE, lw=0.6, ls="--")
    axd.set_ylim(MLD_CAP + 15, MLD_FLOOR - 15)   # depth increases downward
    ...
    ax1.set_ylabel(r"GOP, moving MLD (mmol O$_2$ m$^{-2}$ d$^{-1}$)", fontsize=11,
                    color=GOP_COLOR)
    ax2.set_ylabel(r"NCP (mmol C m$^{-2}$ d$^{-1}$)", fontsize=11, color=NCP_COLOR)
    ...
    fig.suptitle(
        ...
        "Gross Oxygen Production (moving MLD, 50-200 m) vs. the float's own nitrate-budget NCP",
        fontsize=12, y=0.99,
    )
    ax1.set_title(
        "GOP = flux-corrected net diel O$_2$ change + flux-corrected nighttime "
        "loss (Wanninkhof 2014); integration bottom = MLD clamped to [50, 200] m, "
