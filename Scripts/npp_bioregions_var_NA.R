...
#
# North-Atlantic-extent variant: reads the NA CbPM file and uses wider map
# bounds.  Produces a `_NA` suffix on every output filename.
#
# Run from repo root:
#   & "C:/Program Files/R/R-4.5.2/bin/Rscript.exe" Scripts/npp_bioregions_var_NA.R
...
NC_FILE   <- "Output/cmems_cbpm_monthly_NorthAtlantic_0p25deg.nc"
OUT_DIR   <- "Output"
N_BASIS   <- 7                # Fourier basis count (3 harmonics + intercept)
COV_FRAC  <- 7 / 9            # >= 7 valid months out of Feb..Oct
K_RANGE   <- 2:8
N_BOOT    <- 100
DETREND   <- FALSE            # let the basin-scale N->S gradient organize regions
FORCE_K   <- NULL
FORCE_METHOD <- NULL          # NULL = pick best silhouette; else force linkage
SKIP_K2   <- TRUE             # if global silhouette best is K=2, pick best K>=3
MONTH_MID <- seq(0.5, 11.5, by = 1)
REGION_TAG <- "_NA"
# TAG is finalized after K is selected (see section 9); placeholder for now.
TAG <- REGION_TAG
# Map extents for the wider North Atlantic domain (44-68N, -67 to -7W)
MAP_BORDER_X <- c(-70, -5)
MAP_BORDER_Y <- c(42, 70)
MAP_COORD_X  <- c(-68, -7)
MAP_COORD_Y  <- c(44, 69)
...
sil_pool <- sil_table
if (!is.null(FORCE_METHOD)) sil_pool <- sil_pool |> filter(method == FORCE_METHOD)
if (!is.null(FORCE_K)) {
  best <- sil_pool |> filter(K == FORCE_K) |>
    arrange(desc(mean_sil)) |> slice(1)
} else {
  best <- sil_pool |> arrange(desc(mean_sil)) |> slice(1)
  # K=2 on this domain just isolates outlier shelf cells; if it wins
  # globally, fall back to the second-most-informative K (best among K>=3).
  if (SKIP_K2 && best$K == 2) {
    best <- sil_pool |> filter(K >= 3) |> arrange(desc(mean_sil)) |> slice(1)
    message("Global best was K=2; SKIP_K2 -> using second-most-informative ",
            "K = ", best$K)
  }
}
# Finalize self-describing output tag now that K and detrend state are known.
TAG <- sprintf("%s%s_K%d", REGION_TAG, if (!DETREND) "_nodt" else "", best$K)
...
p_map <- ggplot(cell_df) +
  ...
  borders("world", xlim = MAP_BORDER_X, ylim = MAP_BORDER_Y,
          fill = "grey90", colour = "black", linewidth = 0.3) +
  scale_fill_manual(values = clu_palette, name = "Bioregion") +
  coord_quickmap(xlim = MAP_COORD_X, ylim = MAP_COORD_Y) +
