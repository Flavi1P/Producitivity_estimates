# o2_budget_3902681_fullmld.R
# -----------------------------------------------------------------------------
# Diel O2 budget for BGC-Argo float 3902681, integrated to a MOVING bottom depth
# that tracks the FULL mixed-layer depth (MLD), floored at 50 m with NO upper cap
# -- unlike o2_budget_3902681_movingmld.R, which additionally clamps the bottom
# at 200 m.
#
#   z_bot(pair) = max(50, max(MLD_i, MLD_j))
#
# Rationale: the 200 m cap in the _movingmld variant keeps the smoothed summer
# line clean but truncates the deep-convective winter/spring mixed layer, so the
# O2 inventory change of the ventilating layer is under-sampled exactly when the
# ML is deepest. Removing the cap integrates over the actual mixed layer at every
# pair, matching the parent MLD budget in Scripts/o2_budget_3902681.R (which
# floors at 40 m, no upper cap). The 50 m floor is retained to avoid integrating
# over an unrealistically thin summer ML where the diel O2 signal is small and
# the areal budget would collapse.
#
# CAVEAT: with no cap, a pair whose (clamped) bottom is deeper than a profile's
# deepest sampled level yields NA from integrate_o2() and is dropped. This is the
# same behaviour as the parent script and is the price of following the true MLD
# into deep-convection casts that don't reach the bottom of the mixed layer.
#
# The integration bottom is COMMON to the two casts in a pair (max of the two
# MLDs, floored) so the inventory difference inv_j - inv_i is a true change of a
...
# budget in Scripts/o2_budget_3902681.R.
#
...
# is identical to o2_budget_3902681_movingmld.R. MLD is computed per profile
# with castr::mld() (sigma0 0.03 kg m-3 criterion, ref 0-2 m), matching the
# parent script and format_for_ncp.R.
#
# Output: Data/Processed/o2_budget_3902681_fullmld.csv
...
#   & "C:/Users/flapet/AppData/Local/Programs/R/R-4.3.3/bin/x64/Rscript.exe" Scripts/o2_budget_3902681_fullmld.R
# -----------------------------------------------------------------------------
...
out_csv   <- "Data/Processed/o2_budget_3902681_fullmld.csv"

MLD_FLOOR <- 50    # m, shallowest integration bottom (avoid a too-thin summer ML)
# no upper cap: the integration bottom follows the full MLD into deep convection
...
message(sprintf("MLD: median=%.0f m, IQR=[%.0f, %.0f] m, max=%.0f m; floored bottom at %d m, no cap",
                median(prof$mld, na.rm = TRUE),
                quantile(prof$mld, 0.25, na.rm = TRUE),
                quantile(prof$mld, 0.75, na.rm = TRUE),
                max(prof$mld, na.rm = TRUE),
                MLD_FLOOR))
...
# Moving common bottom for the pair = max of the two MLDs, floored (no upper cap).
...
message(sprintf(
  "full MLD: net pairs %d/%d finite (%.0f%%), night pairs %d/%d finite (%.0f%%)",
...
message(sprintf("z_bot used: median=%.0f m, max=%.0f m, at floor(%d) %.0f%%",
                median(budget_out$z_bot_m),
                max(budget_out$z_bot_m),
